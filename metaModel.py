...
class MetaModel_Lasso(customRegressor):
    def __init__(self, in_df, models, sub_params, n_folds, qualPow, imputeDict):
        ...
        self.X = in_df.drop(columns=["SalePrice"]).copy()
        self.y = np.log(in_df)
    ...
    def genPreds(self,X,y):
        ...
        meta_X = pipeline.fit_transform(X)
        self.model_list = [list() for i in self.models]
        folds = KFold(n_splits = self.n_folds, suffle=True, random_state=6)

        oob_preds = np.zeros((X.shape[0], len(self.models)))

        for i, model in enumerate(self.models):
            for trainIdx, outIdx in folds.split(X,y):
                ...
                preds = local_model.predict(X[outIdx])
                oob_preds[outIdx,i] = preds

        self.oob_preds = oob_preds

        # self.meta.fitModel(X, oob_preds, y)

    def fitModel(self,params, cv=5):
        self._params = params

        if not self.predBool:
            genPreds(self.X,self.y)
    ...
    def getTrainRsquared(self):
        ...
        meta_X = np.column_stack(piped_X, self.oob_preds)
        piped_y = self.pipeline_y.fit_transform(self.y)
        return self.model.score(meta_X,piped_y)
        



    def predict(self,X):
        piped_X = self.pipeline.transform(X)
