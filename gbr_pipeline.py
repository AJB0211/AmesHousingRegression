...
class GradBoostReg(customRegressor):
    def __init__(self, in_df, zoning, utilities, frontage, qualPow):

        super(GradBoostReg, self).__init__()
        from lm_features import impute_shell
        ## Because we're currying in python now
        self._imputeVals = impute_shell(
            frontage=frontage, zoning=zoning, utilities=utilities, qualPow=qualPow)
    ...
    def gridSearch(self, params, cv=5, njobs=-1, verbose=50):
        ...
        self._gridSearchObject = GridSearchCV(
            GradientBoostingRegressor(), params, cv=cv, scoring="neg_mean_squared_error", n_jobs=njobs, verbose=verbose)
